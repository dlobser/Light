...
sc1 = {
	...
	setup:function(){
		...
		sc1.swirls = [];
		for(var i = 0 ; i < 5 ; i++){
			var material = facingMat2.clone();//new THREE.MeshLambertMaterial( {map:skyTexture});//facingMat2.clone();
			console.log(i);
			// swirl = Curves[i+""]({
			// 	material:material,
...
makeSurface = function(params){
	...
	swirl.material.side = THREE.BackSide;
	...
	console.log(swirl);
	console.log("fu");
	// var surfAnim = new params.surface();
	// swirl.animation = surfAnim.animation;
	return swirl;
}
