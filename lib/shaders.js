...
var facingFrag2 = "\
	...
	void main(void) {\
		...
		vec4 tex = texture2D(textureColor, (texB.rg*.1)+vec2(vUv.x+offset,vUv.y));\
		vec4 texA = texture2D(textureAlpha, vUv+(tex.rg*.041));\
		vec4 camNorm = vec4(vec3(wNormal),0.) * camMat;\
		gl_FragColor = vec4(vec3(min(1.0,max(0.0,pow(camNorm.z,2.5))))*tex.rgb*texA.a, 1.0);\
	}\
";
