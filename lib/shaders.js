...
// 	void main(void) {\
// 		vec4 texB = texture2D(textureColor, vUv*.1);\
// 		vec4 tex = texture2D(textureColor, (texB.rg*.0051)+vec2(vUv.x*.4+offset,vUv.y));\
// 		vec4 texA = texture2D(textureAlpha, vUv+(tex.rg*.0));\
// 		vec4 camNorm = vec4(vec3(wNormal),0.) * camMat;\
// 		gl_FragColor = vec4(vec3(min(1.0,max(0.0,pow(camNorm.z,1.5))))*tex.rgb*texA.a*tex.a, 1.0);\
// 	}\
// ";
